  ...
  <div style="position:relative">
    ...
    <div class="add-notice"
      ...
      <el-form :model="ruleForm"
        ...
        <el-row v-if="ruleForm.jumpFlag !== 0">
          <el-col :span="16">
        ...
        <el-row v-if="ruleForm.jumpFlag == 2">
          <el-col :span="16">
            <el-form-item label="问卷星问卷链接："
                          prop="linkUrl">
              <el-input size="small"
                        v-model="ruleForm.linkUrl"
                        disabled
                        placeholder="将问卷星调研问卷链接放在此处"
                        style="width:40%;"></el-input>
            </el-form-item>
            <el-form-item>
              <div class="tip"
      ...
      <!-- <div slot="footer" class="footer">
        ...
        <el-button type="danger" size="small" @click="handleSaveDraft()">保存草稿</el-button>
    ...
    <el-dialog title=""
      ...
      <div style="display:flex;justify-content: center;flex-direction: column;align-items: center;">
        ...
        <!-- 图片 -->
        <!-- @click="viewPic()" -->
        <div v-if="ruleForm.imgType == 0">
          <el-image v-show="imgFullUrl"
                    style="width: 200px; height: 300px;display: flex;justify-content: center;"
                    :src="imgFullUrl"
                    fit="contain">
          </el-image>
          <div v-show="!imgFullUrl">暂无封面</div>
        </div>
        <div v-if="ruleForm.imgType == 1">
          <video id="previewVideo"
                 style="width:100%;"
                 v-show="videoFullUrl"
                 :src="videoFullUrl"
                 controls="controls">您的浏览器不支持视频播放</video>
          <div v-show="!videoFullUrl">暂无封面</div>
        </div>
        <!-- 按钮 -->
        <div class="dialog-btn"
             v-if="ruleForm.linkContent">{{ruleForm.linkContent}}</div>
      </div>
    </el-dialog>
    <!-- 查看图片 -->
...
export default {
  data () {
    ...
    const validatorLinkContent = (rule, value, callback) => {
      if (this.ruleForm.jumpFlag != 0) {
        if (value) {
          callback()
        } else {
          callback(new Error('请输入链接按钮文案'))
        }
      } else {
        callback()
      }
    }
    const validatorMenuPath = (rule, value, callback) => {
      if (this.ruleForm.jumpFlag == 1) {
        if (this.ruleForm.menuLv1 && this.ruleForm.menuLv2 && this.ruleForm.menuLv3) {
          callback()
        } else {
          if (!this.ruleForm.menuLv1) {
            callback(new Error('请选择一级菜单'))
          } else if (!this.ruleForm.menuLv2) {
            callback(new Error('请选择二级菜单'))
          } else if (this.ruleForm.menuLv2 == 'function' && !this.ruleForm.menuLv3) {
            callback(new Error('请选择三级菜单'))
          } else {
            callback()
          }
        }
    ...
    return {
      ...
      rules: {
        ...
        visible: [
          { required: true, validator: validatorVisible, trigger: 'change' }
        ],
        jumpFlag: [
          { required: true, validator: validatorJumpFlag, trigger: 'change' }
        ],
        linkContent: [
          { required: true, validator: validatorLinkContent, trigger: 'blur' }
        ],
        linkUrl: [
          { required: true, validator: validatorLinkUrl, trigger: 'blur' }
        ],
        menuPath: [
          { required: true, validator: validatorMenuPath, trigger: 'change' }
        ]
      },
      pickerOptionsStart: {},
      pickerOptionsEnd: {},
  ...
  methods: {
    ...
    visibleChange () {
      ...
      if (this.disabledCheckBoxFlag) { // 选中PES客户去掉直供客户和经销商客户
