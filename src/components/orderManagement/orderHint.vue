...
export default {
  ...
  created () {
    let param = {
      typeName: 'service_article'
    }
    this.requestMethodGet('/web/sys/conf/querySysConfByTypeName', param)
      .then(res => {
        res = res.data
        if (res.code == 1000) {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.loading = true
      this.requestMethodPost('/web/sys/conf/updateSysConfByTypeName', param)
        .then(res => {
          res = res.data
          this.loading = false
          if (res.code == 1000) {
