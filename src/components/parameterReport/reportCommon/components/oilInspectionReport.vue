<template>
  <div :key="refreshOilInspectionKey+1">
    <el-form :model="checkOilForm" ref="checkOilRef" label-width="200px" label-position="left" style="padding:0 20px;">
      ...
      <div v-for="(item,index) in checkOilForm" :key="index">
          ...
          <!-- 油样产品名称 -->
          <el-select v-if="item.type == 'dropdown'&&item.styleJsonDto" size="small" v-model="item.valueCode" placeholder="请选择" clearable filterable remote :loading="loadingRemote" :remote-method="getOilListTile" style="width:220px">
            <el-option v-for="item in oilNameListTile" :key="item.code" :label="item.oilAlias" :value="item.code"></el-option>
          </el-select>
          <el-cascader
            v-if="item.type == 'dropdown'&&!item.styleJsonDto"
            ref='cascaderRef'
            :options="oilNameListCascader"
          ...
          <span slot="label" >{{item.name}}：</span>
          <el-radio-group v-model="item.valueCode" v-if="item.type == 'radio1'" >
                <el-radio v-for="(it,idx) in item.fieldOption" :label="it.value" :key="idx">{{it.option}}</el-radio>
          </el-radio-group>
          </el-form-item>
          </el-col>
          <div>
              <el-col :span="24" v-if="item.type == 'radio1' && item.valueCode == item.children[0].valueCode" v-for="(cylinder,index) in item.children" :key="index">
                <el-form-item :label="cylinder.name+'：'" :required="cylinder.required">
                  <el-date-picker
                    v-if="cylinder.type == 'datepicker3'" 
    ...
    <!-- 展示导入的数据 -->
      <div v-for="(item,index) in iotData" :key="refreshOilInspectionKey" style="padding:20px;background:rgb(249,249,249);border-radius:4px;margin-top:20px;display:flex;align-items: center;">
        <div style="flex:1;">
          <el-row>
            ...
            <div v-for="(it,index) in item.children" :key="index" >
              <el-col :span="20" v-if="it.name == '备注' && it.name !='隐藏字段_iot_id'" style="margin:16px 48px 0px 0;">
                ...
                <span v-if="it.type == 'string'" style="font-family:ArialMt;font-size:13px;color:rgb(51,51,51)">{{it.value}}</span>
              </el-col>
              <el-col :span="6" v-if="it.name != '备注' &&  it.name !='隐藏字段_iot_id'" style="margin:16px 48px 0px 0;">
                <span style="font-family:MicrosoftYaHei;font-size:13px;color:rgb(153,153,153)">{{it.name}}：</span>
                <span v-if="it.type == 'string'" style="font-family:ArialMt;font-size:13px;color:rgb(51,51,51)">{{it.value}}{{it.unit}}</span>
              </el-col>
            </div>
          </el-row>
        </div>
  ...
  export default {
    ...
    created(){
      ...
      this.checkOilForm.map(item=>{
        ...
        if(item.name=='油样产品名称'&&item.styleJsonDto){
          if(item.valueCode){
            this.initOilName(item.valueCode)
          }else{
            this.getOilListTile()
          }
        }
    ...
    methods:{
      async initOilName(code) {
        let resOil = await this.getOilNameByOilCode(code)
        this.oilNameListTile=[{oilAlias: resOil,code:code}]
      },
      rowKey(row) {
        return row.time
      },
      ...
      deleteIot(index) {
        this.$confirm('是否移除该数据？?', '提示', {
            ...
            this.checkOilForm.forEach(it=>{
              if(it.nameKey=="key_import_data"){
      ...
      getOilListTile(val){
        ...
        this.requestMethodGet("/mdm/oil/queryOilListByPage", {oilName:val, status:1, size:200, current:1})
          .then(res => {
            ...
            if (res.code == 1000) {
              this.oilNameListTile = res.data.list
            } else {
              this.$message({
                message: res.msg,
                type: "warning"
      ...
      handleSubmitOilCheck() {
          ...
          ++this.refreshOilInspectionKey
      },
  
      // 更新风机现场验油报告type是dropDown时,同时更新value的值,因为后端校验是根据value字段,小程序也需要根绝value字段回显
      selectChange(val,index){
        let checkOilObj= this.checkOilForm[index]
        if(!val) {
          checkOilObj.value = ''
          return
        }
        let valueObj = this.oilNameList.find(item=>item.code == val)
        checkOilObj.value = valueObj.name
      }
    }
  }
  </script>
