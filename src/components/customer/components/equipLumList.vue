<template>
  <div class="equipment-lubr" style="position:relative">
    <el-button style="margin-bottom: 20px;" type="danger" plain @click="handleDown" size="small">导出设备润滑表</el-button>
    <!-- 表格 -->
    <custom-table
      class="table"
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        oilAddPeriodName: "",
        currentOilCodeName: [],
        everyTimeOilAddQuantityName: "",
        preMonthOilAddQuantityName: "",
        lastOilAddDate: [],
      ...
      tableDataHeaderInit: [
        { slot: "selection" },
        {
          ...
          selectOptions: [{ code: 1, name: "活跃" }, { code: 0, name: "不活跃" }],
          changeValueInput: "",
          changeValueSelect: "",
          label: "设备名称",
          slot: "deviceName",
        ...
        { filterType: "input", paramValue: "lubricationPointName", selectOptions: [], label: "润滑点设备名称", prop: "lubricationPointName", fixed: "left", width: "250", tooltip: true },
        { prop: "lubricationMethod", label: "润滑方式", width: "180", tooltip: true },
        {
          filterType: "cascaderOil",
          selectMultiple: true,
          label: "在用油名称",
          paramValue: "currentOilCodeName",
          changeValue: "",
          prop: "currentOilCodeName",
          selectOptions: [],
          label: "在用油名称",
          width: "180",
          tooltip: true
  ...
  methods: {
    ...
    uploadData() {
      ...
      //所属客户
      queryParams.companyIds = this.ruleForm.id;
      // 油箱容量
      ...
      params.menuCode = "PC_Customer_Lurication"

      this.requestMethodPost("/lubePoint", params)
        .then(res => {
          loading.close();
          let data = res.data;
      ...
      this.requestMethodPost("/lubePoint/getLubePointList", params)
        .then(res => {
          ...
          if (res.code == 1000) {
            ...
            this.$emit("change", this.totalNumber);
