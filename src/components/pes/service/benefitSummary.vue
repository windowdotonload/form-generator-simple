          ...
          <div v-for="(item, index) in resultForm.solutionListJsonDtoList" :key="index">
            <div class="plan-items-title">
              ...
              <el-button
                ...
                v-if="index != 0 && ((statusPlan == '3' && item.type == '1') || statusPlan != '3')"
                type="text"
                size="small"
        ...
        <el-form :model="mlsaObj" ref="mlsaObj" label-width="190px" label-position="right">
          ...
          <el-row v-if="mlsaObj.hasOilFlag" class="service-items-wrap" v-for="(item, index4) in contrastList" :key="index4">
            ...
            <el-col :span="6" align="right">
              ...
              <el-button type="text" @click="deleteAttachment(item)" style="color:#dd0000;font-size:13px;">删除</el-button>
            </el-col>
          </el-row>
        </el-form>
      </div>
      ...
      <div style="padding: 24px 20px 0">
        <el-form :model="otherInfoJsonDto" :rules="rulesOther" ref="otherInfoJsonDto" label-width="190px" label-position="right">
          <el-form-item label="客户领导" prop="customerLd" :rules="rulesOther.customerLd">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.customerLd"></el-input>
          </el-form-item>
          <el-form-item label="客户工程师" prop="customerGcs" :rules="rulesOther.customerGcs">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.customerGcs"></el-input>
          </el-form-item>
          <!-- 创建人为美孚账号，必填 -->
          <el-form-item label="区域销售经理" prop="areaXsjl" v-if="userInfo.userType == '0'" :rules="[{ required: true, message: '请输入区域销售经理', trigger: 'blur' }]">
            <el-input
              size="small"
              maxlength="20"
              style="width: 380px; margin-right: 20px"
          ...
          <!-- 创建人为经销商账号，非必填 -->
          <el-form-item label="区域销售经理" prop="areaXsjl" v-if="userInfo.userType == '1'">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.areaXsjl"></el-input>
          </el-form-item>
          <!-- 创建人为美孚账号，必填 -->
          <el-form-item label="区域技术经理" prop="areaJsjl" v-if="userInfo.userType == '0'"  :rules="[{ required: true, message: '请输入区域技术经理', trigger: 'blur' }]">
            <el-input
              size="small"
              maxlength="20"
              style="width: 380px; margin-right: 20px"
          ...
          <!-- 创建人为经销商账号，非必填 -->
          <el-form-item label="区域技术经理" prop="areaJsjl" v-if="userInfo.userType == '1'">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.areaJsjl"></el-input>
          </el-form-item>
          <el-form-item label="经销商公司" prop="distributorCompany" :rules="rulesOther.distributorCompany">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.distributorCompany"></el-input>
          </el-form-item>
          <el-form-item label="经销商技术服务工程师" prop="distributorJsfwgcs" :rules="rulesOther.distributorJsfwgcs">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.distributorJsfwgcs"></el-input>
          </el-form-item>
          <el-form-item label="经销商销售经理" prop="distributorSxjl" :rules="rulesOther.distributorSxjl">
            <el-input size="small" maxlength="20" style="width: 380px; margin-right: 20px" v-model="otherInfoJsonDto.distributorSxjl"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <el-row class="service-items-wrap">
        <div style="color:#dd0000;font-size:14px;line-height:14px;font-weight:bold;">
          服务执行人所属公司名称：<span style="color:#999;font-weight:400;">{{ $userInfo.orgInfo.companyName }}</span>
        </div>
      </el-row>
      <!-- 附件 -->
      <div class="right-content" style="margin-top: 20px">
    ...
    getTcoData() {
      this.requestMethodGet("/service/report/template/queryTcoTemplate", {}).then(res => {
        if (res.data.code == 1000) {
          ...
          this.showTco = true;
          console.log("this.tcoItem", this.tcoItem,this.showTco);
        } else {
          this.$message({
            message: res.msg,
            type: "warning"
...
// 获取全路径赋值imgFullDtoUrl
    ...
    previewReport() {
      ...
      this.requestMethodGet("/pes/report/xyzj/queryXyzjPath", params).then(res => {
        if (res.data.code == 1000) {
    ...
    // 查询效益总结
    getPlanReport() {
      this.requestMethodGet("/pes/report/xyzj/queryDetailById", { pesInfoId: this.pesId }).then(res => {
        if (res.data.code == 1000) {
          if (res.data.data) {
              ...
              this.$nextTick(() => {
                this.planForm.resultListJsonDtoList.forEach((item, index) => {
                  item.resultAfterImgPathList.forEach((afterUrl, index1) => {
                    this.getFullUrl(afterUrl, index, index1, "after");
                  });
                  item.resultBeforeImgPathList.forEach((beforeUrl, index2) => {
                    this.getFullUrl(beforeUrl, index, index2, "before");
                  });
                });
              });
            } else {
    ...
    // 获取图片全路径
    getFullUrl(url, index, index1, val) {
      const params = {
        url: url
      };
      this.requestMethodGet("/web/file/getFullUrl", params).then(res => {
    ...
    saveInfo(val) {
      ...
      this.requestMethodPost("/pes/report/xyzj/saveUpdate", parmas).then(res => {
        if (res.data.code == 1000) {
          this.disabledPreview = false;
          if (res.data.data) {
    ...
    saveInfoPublish(val) {
      if (val == "draft") {
        ...
        if(this.basicInfoForm.backgroundDesc=="<p><br></p>"||this.basicInfoForm.backgroundLub=="<p><br></p>") {
    ...
    cancleEditInfo() {
      this.$confirm("取消后将丢失本次编辑内容", "取消编辑", {
        ...
        // this.$gio.cancelCreateArchiveReport({operation: '弹窗中点击确定', reportType: '效益总结',customer:this.$route.query.companyNumber})
        })
        .catch(() => {
        // this.$gio.cancelCreateArchiveReport({operation: '弹窗中点击继续编辑', reportType: '效益总结',customer:this.$route.query.companyNumber})
        });
        // this.$gio.cancelCreateArchiveReport({operation: '点击取消按钮', reportType: '效益总结',customer:this.$route.query.companyNumber})
    }
  }
};
</script>
