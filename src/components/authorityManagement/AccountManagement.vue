  ...
  <div style="position:relative;">
    <div class="account-management" :style="mainminheight" id="accountManage">
      <div class="toolbox-con flexBetween">
        ...
        <div class="tool-con">
          <!-- v-has="honortitle" -->
          <el-button type="danger" plain size="small" v-has="honortitle" @click="importHonortitle">批量导入荣誉称号</el-button>
          <el-button plain @click="exportData" size="small" v-has="accountDownBtn">批量导出</el-button>
          <el-button plain @click="importFile" size="small" v-has="accountUpBtn">批量导入</el-button>
          <el-button type="danger" @click="handleAdd()" size="small" v-has="accountAddBtn">新建账号</el-button>
          <custom-field @get-fields="getCustomFields" :customFieldlist="customFieldlist" listType="userAccount" style="display:inline-block;margin-left:10px;"></custom-field>
        </div>
      </div>
      <!-- 表格 -->
      <custom-table
      ...
      >
        ...
        <template v-slot:roleType="slotProps">
          <el-table-column
            ...
            show-overflow-tooltip
          >
            <template slot="header" v-if="slotProps.slotData.paramValue">
              <column-header
                title="角色归属"
                :filterType="slotProps.slotData.filterType"
                :paramValue="slotProps.slotData.paramValue"
            ...
            <template slot-scope="scope">
              <span>{{ scope.row.userType == 0 ? "美孚" : scope.row.userType == 1 ? "经销商" : "客户" }}</span>
            </template>
          </el-table-column>
        </template>
        ...
        <el-table-column slot="operation" :resizable="false" fixed="right" label="操作" width="230" align="center">
          <template slot-scope="scope">
            ...
            <el-link @click="handleEdit(scope.row)" style="margin-left:10px;color:rgb(0,20,79)" v-has="accountEditBtn">编辑</el-link>
            <span>
              <el-link @click="handleTransfer(scope.row)" style="margin-left:10px;color:rgb(0,20,79)" v-has="User_Data_Transfer">数据转移</el-link>
            </span>
            <el-link type="danger" @click="handleDelete(scope.row)" v-has="accountDelBtn" style="margin-left:10px">删除</el-link>
          </template>
        </el-table-column>
      </custom-table>
      <el-pagination
      ...
      <el-dialog :title="dialogTitle" v-if="dialogFormVisible" :visible.sync="dialogFormVisible" :show-close="false" width="700px" :close-on-click-modal="false">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" label-position="top">
          ...
          <el-row>
            <el-col :span="12" v-if="$userInfo.userType == 0">
              <el-form-item label="角色归属" :prop="dialogTitle == '查看账号' ? '' : 'roleType'">
                <span v-if="dialogTitle == '查看账号'" style="color:#000">{{ ruleForm.roleType == 0 ? "美孚" : ruleForm.roleType == 1 ? "经销商" : "客户" }}</span>
                <el-radio-group v-model="ruleForm.roleType" @change="radioChange" v-else>
            ...
            <el-col :span="12" v-if="$userInfo.userType == 1">
              <el-form-item label="角色归属" :prop="dialogTitle == '查看账号' ? '' : 'roleType'">
                <span v-if="dialogTitle == '查看账号'" style="color:#000">{{ ruleForm.roleType == 0 ? "美孚" : ruleForm.roleType == 1 ? "经销商" : "客户" }}</span>
                <el-radio-group v-model="ruleForm.roleType" @change="radioChange" v-else>
                  <el-radio :label="1">经销商</el-radio>
            ...
            <el-col :span="12">
              <el-form-item label="所属公司" :prop="dialogTitle == '查看账号' ? '' : 'companyNumber'">
                <span v-if="dialogTitle == '查看账号'" style="color:#000">{{ ruleForm.companyName }}</span>
                <el-select
                  size="small"
                  v-else
                  clearable
                  ...
                  :disabled="$userInfo.userType == 2 || $userInfo.userType == 1"
                  placeholder="请选择"
                  style="width:280px"
                  @change="
                    val => {
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="相关经销商" prop="spId" v-if="SPList.length > 1 && ruleForm.roleType == 2 && dialogTitle == '新建账号'">
                <div class="selectFollow">
                  <el-select size="small" popper-class="myCascader" :popper-append-to-body="false" clearable filterable v-model="spId" placeholder="请选择" style="width: 280px">
            ...
            <el-col :span="12" v-if="ruleForm.roleType == 2">
              <el-form-item label="所属岗位" prop="postIdList" key="postIdList">
                <span v-if="dialogTitle == '查看账号'" style="color:#000">{{ ruleForm.postName }}</span>
            ...
            <el-col :span="12" v-if="ruleForm.roleType == 0">
              <el-form-item label="区域" :prop="dialogTitle == '查看账号' ? '' : 'area'" key="area">
      ...
      <el-dialog :visible.sync="showImportResultsFlag" width="35%" :close-on-click-modal="false">
        ...
        <div v-if="importType == 'account'">
          ...
          <div style="padding-left:30%;line-height:40px;margin-top:30px;" v-else>
            <div>本次导入数据共 {{ importResult.allCount }} 条</div>
...
export default {
  ...
  data() {
    ...
    var validateEmail = (rule, value, callback) => {
      ...
      } else if (value && this.rules.email[0].required && !value.endsWith("@exxonmobil.com") && !value.endsWith("@exxonmobil.com.cn") && baseRemoteOauth2 == "https://mobilserv.mobil.com.cn/msdp") {
    ...
    return {
      ...
      tableDataHeaderInit: {
        ...
        companyName: { filterType: "input", paramValue: "", changeValue: "", label: "所属公司", prop: "companyName", width: "200", tooltip: true },
        roleType: {
          filterType: "selection",
          paramValue: "",
          selectMultiple: true,
          changeValue: "",
          selectOptions: [{ name: "美孚", code: "0" }, { name: "经销商", code: "1" }, { name: "客户", code: "2" }],
          slot: "roleType",
          label: "角色归属",
          width: "100"
        },
        roleNames: { slot: "roleNames", label: "角色", width: "150", tooltip: true },
        // deptName: { filterType: "input", paramValue: "deptName", changeValue:"", label: "部门", prop: "deptName", width: "120", tooltip: true },
        postName: { filterType: "input", paramValue: "postName", changeValue: "", label: "岗位", slot: "postNames", prop: "postNames", width: "120", tooltip: true },
        inviterName: { slot: "inviterName", label: "邀请人", width: "150", tooltip: true },
  ...
  methods: {
    async getSpList(val) {
      if(this.ruleForm.roleType != 2) return;
      if (this.$userInfo.userType == 1) {
        return (this.spId = this.$userInfo.orgInfo.companyId);
      }
      this.SPList = [];
      ...
      if (this.SPList.length == 1) return (this.spId = this.SPList[0].spId);
    },
    async getWorkShopList(accountIds) {
    ...
    handleTableHeight() {
      setTimeout(() => {
        ...
        this.tableHeight = Number(this.accountManageHeight) - Number(this.formDivHeight) - 40;
      }, 200);
    },
    // 获取自定义字段信息
    getCustomFieldList() {
      ...
      if (fieldDtoList.length == 0) {
        ...
        let index = fieldDtoList.findIndex(item => item.listType == "userAccount");
        if (index != -1) {
      ...
      if (this.$userInfo.userType != 2) {
        this.tableDataHeader.forEach(item => {
          if (item.prop == "companyName") {
            item.paramValue = "companyName";
          }
          if (item.label == "角色归属") {
            item.paramValue = "roleType";
          }
        });
      }
    ...
    confirmTrasfer() {
      ...
      if (this.transferUserNumber) {
        ...
        if (this.replaceObj.roleType != 1) {
          ...
          selectData = this.transferList.filter(item => {
            return item.id == this.transferUserNumber;
          });
        }
    ...
    confirmTrasferTips() {
      ...
      if (this.replaceObj.roleType == 0) {
        ...
        this.requestMethodPost("/companyUser/replaceUserData", param)
          ...
          .catch(res => {});
      } else if (this.replaceObj.userType == 2) {
        let param = {
    ...
    // 所属公司 change 事件
    seleceCompany(val) {
      let company = this.customerList.filter(it => {
        return it.companyNumber == val;
      });
      this.ruleForm.workshopIdList = [];
      this.currentSelectCompany = company[0];
      ...
      this.queryPostList(null, this.companyId);
    ...
    handleEdit(row, name) {
      ...
      this.ruleForm = {
        ...
        postName: row.postNames ? row.postNames.join(",") : "",
        ...
        orgInfo: row.orgInfo,
        companyNumber: row.orgInfo ? row.orgInfo.companyNumber : row.companyNumber, // 使用期限
        companyName: row.orgInfo ? row.orgInfo.companyName : row.companyName,
      ...
      this.getWorkShopList(row.orgInfo.companyId);
      this.$_queryRoleData(); // 获取角色列表
      this.$_queryCustomerData("");
      ...
      this.queryPostList(null, row.orgInfo.companyId);
      this.dialogFormVisible = true;
      if (name) {
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          params.userType = this.ruleForm.roleType;
          if (this.$userInfo.userType == 2) {
            params.orgInfo = {
          ...
          params.orgInfo = Object.assign({}, params.orgInfo, this.currentSelectCompany);
          ...
          if(!params.workshopIdList) params.workshopIdList = []
          this.requestMethodPost(url, params)
            .then(res => {
              ...
              if (res.code == 1000) {
                ...
                this.currentSelectCompany = null;
                this.$_queryAccountList();
              } else {
                this.$message({
                  message: res.msg,
    ...
    uploadTable(row) {
      const params = {
        url: "user/template/批量导入账号模板.xls"
      };
      if (this.$userInfo.userType == "2") {
        params.url = "user/template/客户批量导入账号模板.xls";
      } else if (this.$userInfo.userType == "1") {
        params.url = "user/template/经销商批量导入账号模版.xls";
      }
      this.requestMethodGet("/web/file/getFullUrl", params).then(res => {
        if (res.data.code == 1000) {
