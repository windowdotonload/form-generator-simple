 ...
 * @Autor: LiQiuPing
 * @Date: 2022-10-28 11:27:06
 * @LastEditors: LiQiuPing
 * @LastEditTime: 2022-11-08 11:30:04
-->
<template>
  <div>
                      ...
                      >
                          ...
                          <div class="i-dialog-footer" style="height: 40px">
                              ...
                              <div class="btn-group fl">
                                <el-button size="small" @click="chooseImg" type="danger" plain>选择图片</el-button>
                              </div>
                              <div class="btn-group fr">
                                <el-button size="small" @click="handleClose">取消</el-button>
                                <el-button type="primary" size="small" style="margin-left: 15px" :disabled="!drawImg.img"  @click="cropPicture(false)">确定</el-button>
                              </div>
                          </div>
                      </div>
                  </transition>
...
data () {
  ...
  return {
    ...
    realFileType:null //上传图片之后获取的图片类型
  }
},
mounted () {
  // 是否为弹窗模式
...
methods: {
  ...
  putImgToCanv (e) {
    ...
    if (file) {
      ...
      this.realFileType = file.type //图片画到画布上之后的获取的图片类型
  ...
  cropPicture (doNotReset) {
    let _this = this
    let fileType = _this.realFileType ? _this.realFileType : `image/${_this.fileType}`
    let suffixName = _this.fileType
    if(_this.realFileType){
      suffixName = _this.realFileType.split('/')[1]
    }
    if (this.drawImg.img) {
      // get img
              ...
              setTimeout(() => {
                ...
                try {
                  ...
                  if (e.name == 'TypeError' && window.BlobBuilder) {
                    ...
                    NewBlob = bb.getBlob(fileType)
                  }
                  if (e.name == 'InvalidStateError') {
                    NewBlob = new Blob([arr.buffer], { type: fileType })
                  }
                }
      ...
      canvas.toBlob((blob) => {
        if (blob) {
              ...
              if (reader.readyState == 2) {
                ...
                newCanv.toBlob(
                  (newBlob) => {
                    ...
                    let fileName = _this.changeFileName(_this.fileName, suffixName)
                    if (!doNotReset) {
                      _this.handleClose()
                      // 1Mb
                      if(newBlob.size > 1 * 1024 * 1024){
                        let baseUrl =  newCanv.toDataURL(fileType,1)
                        _this.compressImage(fileName,baseUrl,newBlob)
                      }else{
                        _this.$emit('cutDown', {
                          index: _this.index,
                          fileName,
                          ...
                          file: _this.dataURLtoFile(
                            newCanv.toDataURL(fileType, _this.quality),
                            fileName
                          ),
          ...
          if (!doNotReset) {
            ...
            _this.$emit('cutDown', {
              fileName,
              dataURL: newCanv.toDataURL(fileType, _this.quality)
            })
          } else {
            _this.$emit('onPrintImg', {
              fileName,
  ...
  compressImage(fileName,baseUrl,newBlob){
    let image  = new Image()
    image.src = baseUrl;
    let quality = 0.6 
    let _this = this
    ...
    image.onload = function () {
      ...
      let smallBase64 = canvas.toDataURL(fileType, quality)
      _this.$emit('cutDown', {
        index: _this.index,
        fileName,
        blob: newBlob,
        file: _this.dataURLtoFile(
          canvas.toDataURL(fileType,quality),
          fileName
        ),
        dataURL: smallBase64
      })
